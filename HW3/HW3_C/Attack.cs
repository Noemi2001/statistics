            ...
            while (this.atkForm.running)
            {
                ...
                if (this.picking != -1 && this.picking < this.nAttacks)
                {
                    ...
                    for (int i = 0; i < this.mSystems; i++)
                    {
                        ...
                        for (var c = 0; c < this.intervalsNumber; c++)
                        {
                            ...
                            if (newZero.X == this.aHistogram) 
                            {
                                if (newZero_Y >= (inizio * 10) && newZero_Y < (fine * 10))
                                {
                                    countZero_points[c] += 1;
                                }
                                else if ( newZero_Y == fine && newZero.X == this.nAttacks)
                                {
                                    countZero_points[c] += 1;
                                }
                            }

                            if (newDivided.X == this.aHistogram)
                            {
                                if (newDivided_Y >= (inizio * 10) && newDivided_Y < (fine * 10))
                                {
                                    countDivided[c] += 1;
                                }
                                else if (newDivided_Y == fine && newDivided.X == nAttacks)
                                {
                                    countDivided[c] += 1;
                                }
                            }

                            if (newNormal.X == this.aHistogram)
                            {
                                if (newNormal_Y >= (inizio * 10) && newNormal_Y < (fine * 10))
                                {
                                    countNormalized[c] += 1;
                                }
                                else if (newNormal_Y == fine && newNormal.X == this.nAttacks)
                                {
                                    countNormalized[c] += 1;
                                }
                            }

                            if (newZero.X == this.nAttacks)
                            {
                                if (newZero_Y >= (inizio * 10) && newZero_Y < (fine * 10))
                                {
                                    countZero_points_end[c] += 1;
                                }
                                else if (newZero_Y == fine && newZero.X == this.nAttacks)
                                {
                                    countZero_points_end[c] += 1;
                            ...
                            if (newDivided.X == this.nAttacks)
                            {
                                if (newDivided_Y >= (inizio * 10) && newDivided_Y < (fine * 10))
                                {
                                    countDivided_end[c] += 1;
                                }
                                else if (newDivided_Y == fine && newDivided.X == this.nAttacks)
                                {
                                    countDivided_end[c] += 1;
                                }
                            }

                            if (newNormal.X == this.nAttacks)
                            {
                                if (newNormal_Y >= (inizio * 10) && newNormal_Y < (fine * 10))
                                {
                                    countNormalized_end[c] += 1;
                                }
                                else if (newNormal_Y == fine && this.nAttacks == newNormal.X)
                                {
                                    countNormalized_end[c] += 1;
                                }
                            }
                ...
                try
                {
                    this.atkForm.Invoke(new MethodInvoker(delegate ()
                    {
                        ...
                        {
                            int x = this.aHistogram;
                            int y = -(int)(item.Key*0.5);
